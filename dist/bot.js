...
exports.botPickSplit = botPickSplit;
exports.botShouldStop = botShouldStop;
const game_1 = require("./game");
/** Pick best split for the bot. Returns index 0-2. */
function botPickSplit(room) {
...
function needsNewMarkerPenalty(sums, room) {
    const active = Object.keys(room.activeMarkers).length;
    if (active < game_1.MAX_ACTIVE_MARKERS)
        return 0;
    let pen = 0;
    for (const s of sums) {
        if (!(s in room.activeMarkers))
            pen++;
    }
    return pen;
}
function centerScore(sums) {
...
function botShouldStop(room, player) {
    ...
    for (const colStr of active) {
        ...
        if (room.activeMarkers[col] >= game_1.COLUMN_LENGTHS[col])
            return true;
    }
    ...
    if (active.length === game_1.MAX_ACTIVE_MARKERS) {
        if (totalAdvance >= 4)
            return true;
